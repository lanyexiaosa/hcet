...
def build_model(options):
   ...
   # dx_visit = tf.reshape(dx_visit, (-1, options['dx_emb_size'])) # dx_emb_size=200
    ...
    W_dx = snt.Sequential([snt.Linear(output_size=options['ehr_emb_size'], name='W_ehr'), order_activation])
    EHR_obj = W_dx(EHR_obj)
    
#     # adding mask to sum of codes in this visit
...
def train(input_path,
    ...
    options = locals().copy()
    #build the model
    input_PHs, label_PHs, mask_PHs, loss_T, seq_length_PH, preds_T = build_model(options)
    ...
    with tf.Session() as sess:
        ...
        for train_iter in range(options['num_iter']+1):
            ...
            if train_iter > 0 and train_iter % options['eval_period'] == 0:
                ...
                print('Epoch: %d, valid_loss:%f, valid_auc:%f, valid_aucpr:%f' % (train_iter, valid_loss, valid_auc,
                                                                                 valid_aucpr))
